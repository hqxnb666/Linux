/*******************************************************************************
 * Simple FIFO Order Server Implementation with Queue Management
 *
 * Description:
 *     A server implementation designed to process client requests in First In,
 *     First Out (FIFO) order. The server binds to the specified port number
 *     provided as a parameter upon launch. The parent process handles receiving
 *     requests and adding them to a shared queue, while the worker thread
 *     processes the requests from the queue.
 *
 * Usage:
...
void handle_connection(int conn_socket) {
    ...
    if (pthread_create(&worker_thread, NULL, worker_main, (void *)q) != 0) {
...
int main(int argc, char **argv) {
    ...
    /* Close the listening socket (unreachable in this code) */
    close(sockfd);
    return 0;
}
