...
{
    char filename[256];
    snprintf(filename, sizeof(filename), "retrieved_image_%ld.bmp", request_data.request.req_id);

    if(saveBMP(filename, source_image) == 0) {
        sync_printf("INFO: The image has been successfully saved as %s\n", filename);
    } else {
