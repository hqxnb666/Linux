...
/* Creates a new image by rotating the input image by 90 degrees
...
struct image* loadBMP(const char* filename) {
	...
	if (read(fd, &header, sizeof(BMPHeader)) != sizeof(BMPHeader)) {
	    close(fd);
	    return NULL;
	}
	if (read(fd, &infoHeader, sizeof(BMPInfoHeader)) != sizeof(BMPInfoHeader)) {
	    close(fd);
	    return NULL;
	}
	...
	do {
		for (x = 0; x < infoHeader.width; x++) {
			...
			if (read(fd, color, sizeof(unsigned char) * 3) != 3) {
			    deleteImage(img);
			    close(fd);
			    return NULL;
			}
	...
	} while (y-- > 0); /* The post-decrement here is important not
...
uint8_t saveBMP(const char* filename, const struct image* img) {
	...
	if (write(fd, &header, sizeof(BMPHeader)) != sizeof(BMPHeader)) {
	    close(fd);
	    return 1;
	}
	if (write(fd, &infoHeader, sizeof(BMPInfoHeader)) != sizeof(BMPInfoHeader)) {
	    close(fd);
	    return 1;
	}
	...
	do {
		for (x = 0; x < img->width; x++) {
			...
			unsigned char color[3] = { pixel & 0xFF,
						   (pixel >> 8) & 0xFF,
						   (pixel >> 16) & 0xFF };
			if (write(fd, color, sizeof(unsigned char) * 3) != 3) {
			    close(fd);
			    return 1;
			}
		}
	...
	} while ( y-- > 0); /* The post-decrement here is important
...
uint8_t sendImage(struct image* img, int sockfd) {
    ...
    while (to_send) {
	    ssize_t cur = send(sockfd, bufptr, to_send, 0);
	    if (cur <= 0) {
...
struct image * recvImage(int sockfd) {
	...
	ssize_t received = recv(sockfd, magic, 3, 0);
	if (received != 3 || strncmp(magic, "IMG", 3) != 0) {
		return NULL;
	}
	...
	size_t to_recv = img->width * img->height * sizeof(uint32_t);
	char * bufptr = (char *)(img->pixels);

	/* Receive all the pixel bytes on the socket */
	while(to_recv > 0) {
		ssize_t cur = recv(sockfd, bufptr, to_recv, 0);
		if (cur <= 0) {
			deleteImage(img);
			return NULL;
		}
